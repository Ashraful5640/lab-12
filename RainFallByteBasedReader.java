...
public class RainFallByteBasedReader {
	...
	public static void main(String[] args) {
		...
		try {
			...
			DataInputStream disIn = new DataInputStream(new FileInputStream(sourceFile));//DI class
			...
			while(disIn.available() > 0) {
				...
				stations.add(station);//add(parametreized method)
			...
			System.out.println("There are " + stations.size() + " stations and "
						+ districtCount.size() + " districts in Melaka.");//size () mmetod
